...
public class ArrList <E> {
	private  E a[];     // 리스트의 항목들을 저장할 배열 데이터 타입
    private  int size;  // 리스트의 항목 수
   
    public  ArrList() { // 생성자
        a = (E[]) new Object[1];  // 최초로 1개의 원소를 가진 배열 생성(배열은 size가 고정되어야한다)
        size = 0;                 // 항목 수를 0으로 초기화
    } // java로 배열만들기
    public boolean isEmpty() {return size == 0;} // 리스트가 empty이면 true 리턴 앞에 is가 붙으면 true or false
    ...
    private void resize(int newSize) {		// 배열 크기 조절
		Object[] t = new Object[newSize];   // newSize 크기의 새로운 배열 t 생성 Object는 최상위 클래스
